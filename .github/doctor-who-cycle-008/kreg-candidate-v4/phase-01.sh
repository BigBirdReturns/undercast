...
# The published carrier remains one direct commit over its frozen floor, while
# the PR may target a later disjoint main descendant.
...
test "$(wc -l < "$OUT/expected-carrier-paths.txt")" = "23"
...
# Re-fetch the PR rather than trusting only event fields.
...
OUT="$OUT" HEAD_SHA="$head_sha" BASE_SHA="$base_sha" python3 - <<'PY'
...
assert pr['head']['ref']=='agent/doctor-who-cycle-008-kreg-candidate-v5'
assert pr['head']['sha']==os.environ['HEAD_SHA']
assert pr['base']['ref']=='main' and pr['base']['sha']==os.environ['BASE_SHA']
assert pr['commits']==1 and pr['changed_files']==len(expected)==23
