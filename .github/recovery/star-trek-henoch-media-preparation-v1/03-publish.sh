...
jq \
  ...
  "$OUT/media-receipt.json" > "$OUT/media.tmp"
mv "$OUT/media.tmp" "$OUT/media-receipt.json"
...
mkdir -p "$PUBLISH/transport/star-trek-henoch-media-v1"
cp -a "$OUT"/. "$PUBLISH/transport/star-trek-henoch-media-v1/"
...
export GIT_AUTHOR_NAME=undercast-henoch-media-v1
export GIT_AUTHOR_EMAIL=undercast-henoch-media-v1@users.noreply.github.com
export GIT_COMMITTER_NAME="$GIT_AUTHOR_NAME"
export GIT_COMMITTER_EMAIL="$GIT_AUTHOR_EMAIL"
result_commit="$(
  printf '%s\n' "Record Star Trek Henoch media preparation v1" \
...
git show \
  "${result_commit}:transport/star-trek-henoch-media-v1/media-receipt.json" \
  | jq -e \
