...
NASA_PAGE='https://images.nasa.gov/details-E67-16643-3'

def get_json(url):
 last=None
 for attempt in range(1,9):
  try:
   req=urllib.request.Request(url,headers={'User-Agent':'undercast-cadmar-media-scout/2.1'})
   with urllib.request.urlopen(req,timeout=120) as r: return json.loads(r.read())
  except (urllib.error.HTTPError,urllib.error.URLError) as e:
   last=e; time.sleep(attempt*3)
...
def download(url,path):
 ...
 for attempt in range(1,9):
  try:
   req=urllib.request.Request(url,headers={'User-Agent':'undercast-cadmar-media-scout/2.1','Accept':'image/*'})
   with urllib.request.urlopen(req,timeout=120) as r: path.write_bytes(r.read()); return
  except (urllib.error.HTTPError,urllib.error.URLError) as e:
   last=e; time.sleep(attempt*4)
...
asset=get_json(f'https://images-api.nasa.gov/asset/{NASA_ID}')
hrefs=[row.get('href') for row in asset.get('collection',{}).get('items',[]) if row.get('href')]
images=[u for u in hrefs if u.lower().split('?')[0].endswith(('.jpg','.jpeg','.png','.tif','.tiff'))]
if not images: raise RuntimeError(f'NASA asset has no image rendition: {hrefs}')
rank=lambda u: (0 if '~orig.' in u else 1 if '~large.' in u else 2 if '~medium.' in u else 3 if '~small.' in u else 4, len(u))
selected=sorted(images,key=rank)[0]
p=OUT/'dryden-group.jpg'; download(selected,p)
with Image.open(p) as im:
 ...
 rows=[{'id':'dryden-group','file':p.name,'width':w,'height':h,'sha256':hashlib.sha256(p.read_bytes()).hexdigest(),'title':TITLE,'page':PAGE,'nasa_page':NASA_PAGE,'nasa_id':NASA_ID,'selected_nasa_asset':selected,'all_nasa_assets':hrefs,'author':'NASA','license':'Public domain'}]
...
(OUT/'manifest.json').write_text(json.dumps({'version':2,'transaction':'STAR-TREK-CADMAR-MEDIA-SCOUT-V2','canonical_parent':EXPECTED_MAIN,'items':rows},indent=2)+'\n')
