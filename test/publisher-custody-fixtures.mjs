...
function exactTerminalFixture() {
  ...
  return { expected, settlement, pr };
}

fixture("exact terminal publication passes after product PR re-fetch", () => {
  const { expected, settlement, pr } = exactTerminalFixture();
  validateTerminalPublication({ settlement, pr, current_main_sha: expected.base_sha }, expected);
});
...
mustThrow("main advancement immediately before closure fails closed", () => {
  const { expected, settlement, pr } = exactTerminalFixture();
  validateTerminalPublication({ settlement, pr, current_main_sha: "f".repeat(40) }, expected);
