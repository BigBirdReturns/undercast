...
rm -rf "$worktree"
git worktree add --detach "$worktree" "$qualification_parent"
...
test "$(git -C "$worktree" show -s --format=%P "$integration_commit")" = "$qualification_parent"
...
cat > "$evidence/integration-receipt.json" <<EOF
{
  ...
  "lane_base": "$lane_base",
  "integration_base": "$qualification_parent",
