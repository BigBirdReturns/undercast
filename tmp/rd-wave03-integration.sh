...
[[ "$(git show -s --format=%P "$GITHUB_SHA")" == "$base" ]]
expected_controller=(
  .github/workflows/rd-wave03-hosted-executor.yml
  tmp/rd-wave03-integration.sh
)
mapfile -t actual_controller < <(git diff --name-only "$base" "$GITHUB_SHA" | LC_ALL=C sort)
...
[[ "$(git -C "$worktree" show -s --format=%P "$integration_commit")" == "$base" ]]
