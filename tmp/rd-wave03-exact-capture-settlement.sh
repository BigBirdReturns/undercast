...
# Recover the reviewed generator source from its quarantined source branch.
...
text=path.read_text()
old="assert(JSON.stringify(refusals) === JSON.stringify(EXPECTED_REFUSALS), 'refusal denominator changed');"
new="for (const [key, count] of Object.entries(EXPECTED_REFUSALS)) assert(refusals[key] === count, `refusal denominator changed for ${key}`); assert(Object.keys(refusals).length === Object.keys(EXPECTED_REFUSALS).length, 'unexpected refusal class');"
if text.count(old) != 1:
    raise SystemExit(f'expected one refusal comparison, found {text.count(old)}')
