...
(() => {
  ...
  const normalizeArchiveCurrent = () => {
    const primaryCurrent = document.querySelector('.site-nav [aria-current="page"]');
    if (!primaryCurrent) return;
    for (const current of document.querySelectorAll('.archive-map [aria-current="page"]')) {
