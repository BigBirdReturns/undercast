...
const CONTROL='.github/CARD-BACKFILL-UC-047.json';
const OUT=process.env.OUT||'/tmp/card-backfill-uc-047';
...
assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-047','UC-047 control drift');
assert(control.actor==='Lon Chaney Jr.'&&control.character==='The Wolf Man'&&control.side==='still','UC-047 identity boundary drift');
...
try{
  ...
  assert(mime==='image/jpeg'&&bytes.length>100000,`Commons asset unusable ${bytes.length} ${mime}`);
  const candidatePath=join(OUT,'uc-047-still-candidate.jpg');
  await writeFile(candidatePath,bytes);
  const dimensions=identify(candidatePath);
  assert(dimensions.width>=400&&dimensions.height>=500,`Commons source too small ${dimensions.width}x${dimensions.height}`);
  const candidate={path:'uc-047-still-candidate.jpg',mime,bytes:bytes.length,sha256:sha(bytes),...dimensions};
  ...
  assert(cropTop>=0&&cropTop+control.crop.height<=resized.height,'crop offset outside resized image');
  ...
  const duplicateScan={version:1,repository_hash_count:repository.size,items:[{label:'Lon Chaney Jr. as the Wolf Man source / candidate',path:candidate.path,sha256:candidate.sha256,matches:repository.get(candidate.sha256)||[]},{label:'card crop preview',path:crop.path,sha256:crop.sha256,matches:repository.get(crop.sha256)||[]}]};
  ...
  const manifest={version:1,lane:'card-backfill',record_id:'UC-047',actor:'Lon Chaney Jr.',character:'The Wolf Man',production:'The Wolf Man',year:1941,side:'still',expected_subject:'The Wolf Man',generated_at:new Date().toISOString(),control_sha256:sha(await readFile(CONTROL)),source:{provider:control.source.provider,source_page:control.source.source_page,file_title:control.source.file_title,resolved_asset_url:response.url()||assetUrl,api_status:apiStatus,author:strip(meta.Artist?.value)||'Universal Studios, re-released by Realart',license:strip(meta.LicenseShortName?.value)||'Public domain',description:strip(meta.ImageDescription?.value)||"Lobby card for Universal Studios' The Wolf Man featuring Lon Chaney Jr. as the title character.",timestamp:info?.timestamp||null,commons_sha1:info?.sha1||null,page_evidence:pageEvidence},candidate,crop_preview:crop,duplicate_scan:{path:'duplicate-scan.json',repository_hash_count:repository.size,status:'pass'},disposition:'candidate-only-pending-exact-subject-review',canonical_mutation:false};
  await writeJson(join(OUT,'manifest.json'),manifest);
  await writeJson(join(OUT,'review.json'),{version:1,record_id:'UC-047',actor:'Lon Chaney Jr.',character:'The Wolf Man',side:'still',expected_subject:'The Wolf Man',candidate_sha256:candidate.sha256,crop_preview_sha256:crop.sha256,identity_ruling:'pending',presentation_ruling:'pending',crop_ruling:'pending',canonical_mutation:false,disposition:'candidate-only-pending-exact-subject-review'});
  await writeFile(join(OUT,'review.md'),`# UC-047 still candidate pending exact-subject review\n\n- **Record:** UC-047\n- **Performer:** Lon Chaney Jr.\n- **Displayed role:** The Wolf Man\n- **Source:** [Wikimedia Commons](${control.source.source_page})\n- **Candidate:** \`${candidate.sha256}\`\n- **Wall-crop preview:** \`${crop.sha256}\`\n- **Source license:** ${manifest.source.license}\n- **Canonical mutation:** none\n\nThe exact Commons page and source bytes identify Lon Chaney Jr. as the Wolf Man. Visual review must still confirm the expected designed character and crop before evidence sealing.\n`);
  console.log(`GATHERED UC-047 candidate ${candidate.sha256} ${candidate.width}x${candidate.height}`);
