...
async function main() {
  ...
  await writeFile(`${out}/composite-census.md`, [
    ...
    "## Classes",
    "",
    ...summary.census_class.map((row) => `- ${row.key}: ${row.count}`),
    "",
    "## Obligations",
    "",
    ...obligations.map((row) => `- ${row.obligation_id}: **${row.census_class}** — ${row.expected_subject} — ${row.actor || "unknown actor"} — ${row.production || "unknown production"}`),
    "",
  ].join("\n"));
  console.log(`PASS — classified ${obligations.length} repository-grounded composite obligations`);
  console.log(`CLASSES — ${JSON.stringify(Object.fromEntries(summary.census_class.map((row) => [row.key, row.count])))}`);
