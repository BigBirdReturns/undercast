...
const CONTROL='.github/CARD-BACKFILL-UC-066-RENDER.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const OUT=process.env.OUT||'/tmp/card-backfill-uc-066-render';
...
assert(control.version===1&&control.record_id==='UC-066'&&control.actor==='John Hurt'&&control.side==='still','UC-066 render scope drift');
assert(control.face_artifact?.artifact_id===8636067244&&control.face_artifact?.head_sha==='3c6c453e49a4348d800da64b80c7a9b1f2d65ad0','UC-066 face artifact custody drift');
assert(control.ruling?.identity==='expected-subject'&&control.ruling?.presentation==='character-depiction'&&control.ruling?.crop_ruling==='pass-center-focus'&&control.ruling?.canonical_mutation===false,'UC-066 ruling drift');
assert(control.crop?.gravity==='center'&&control.rejected_visual_duplicates?.length===4,'UC-066 crop or visual-duplicate boundary drift');

const sourceManifestPath=join(SOURCE_ROOT,'manifest.json');
const sourceManifestBytes=await readFile(sourceManifestPath);
assert(sha(sourceManifestBytes)===control.selection.source_manifest_sha256,'face manifest custody drift');
const sourceManifest=JSON.parse(sourceManifestBytes.toString('utf8'));
assert(sourceManifest.record_id==='UC-066'&&sourceManifest.actor==='John Hurt'&&sourceManifest.character==='John Merrick','face manifest identity drift');
const selected=(sourceManifest.candidates||[]).find(row=>row.local===control.selection.artifact_path);
assert(selected,'authorized Merrick candidate absent from face manifest');
assert(selected.provider===control.selection.provider&&selected.source_key===control.selection.source_key&&selected.pinned===true,'selected source identity drift');
...
assert(selected.label===control.selection.source_caption,'selected source caption drift');
assert(selected.mime===control.selection.mime&&selected.bytes===control.selection.bytes&&selected.width===control.selection.width&&selected.height===control.selection.height&&selected.sha256===control.selection.sha256,'selected candidate receipt drift');
assert(Array.isArray(selected.repository_matches)&&selected.repository_matches.length===0,'selected candidate duplicates canonical media');
const pageEvidence=sourceManifest.page_evidence?.[control.selection.source_key];
assert(pageEvidence?.status==='loaded'&&pageEvidence?.http_status===200&&pageEvidence?.required_terms_missing?.length===0,'selected source page evidence incomplete');
assert(String(pageEvidence.body_text||'').includes('John Hurt as Joseph Merrick')&&String(pageEvidence.body_text||'').includes('The Elephant Man'),'selected source caption evidence drift');
...
assert(signatureMime(sourceBytes)==='image/jpeg'&&sourceBytes.length===control.selection.bytes&&sha(sourceBytes)===control.selection.sha256,'source byte custody drift');
...
const sourceDimensions=identify(sourcePath);assert(sourceDimensions.width===800&&sourceDimensions.height===450,'source geometry drift');

await rm(OUT,{recursive:true,force:true});await mkdir(OUT,{recursive:true});
const candidatePath=join(OUT,'uc-066-still-candidate.jpg'),pageDest=join(OUT,'source-page.png');
