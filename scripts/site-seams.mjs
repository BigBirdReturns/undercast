...
// one frame, which only reads as a morph for a matched pair (effectively just
// Morn). The reveal is now the trading-card flip / the side-by-side plates —
// whole image at a time, which works for every pair. Guard the seam stays gone
// and the record keeps its character/person reveal.
expect(!has("recognition.html", /uc-wipe|comparison-stage|Compare in one frame/), "recognition: retired comparison seam returned");
