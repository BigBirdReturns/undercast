...
async function inspectBrowserPage(context, pageSpec) {
  ...
  try {
    ...
    await page.screenshot({ path: join(OUT, screenshot), fullPage: true }).catch(() => {});
...
async function inspectTextEndpoint(context, pageSpec) {
  if (!pageSpec.text_url) return null;
  try {
    const response = await context.request.get(pageSpec.text_url, { headers: { 'User-Agent': UA, Referer: pageSpec.url, Accept: 'application/json,text/javascript,text/plain,*/*;q=0.2' }, timeout: 90000, failOnStatusCode: false });
    const bytes = Buffer.from(await response.body());
    const text = bytes.toString('utf8');
    const path = `pages/${pageSpec.key}-product.txt`;
    await mkdir(join(OUT, 'pages'), { recursive: true });
    ...
    try {
      ...
      for (const image of parsed.images || []) {
        const value = typeof image === 'string' ? image : image?.src;
        if (value) rows.push({ url: value, label: parsed.title || '', context: `${parsed.title || ''} ${parsed.description || ''}`, origin: 'product-json-image' });
      }
      const featured = typeof parsed.featured_image === 'string' ? parsed.featured_image : parsed.featured_image?.src;
      if (featured) rows.push({ url: featured, label: parsed.title || '', context: `${parsed.title || ''} ${parsed.description || ''}`, origin: 'product-json-featured' });
    } catch {}
    return { status: response.ok() ? 'loaded' : 'error', http_status: response.status(), resolved_url: response.url() || pageSpec.text_url, text, text_path: path, text_sha256: sha(bytes), rows };
  } catch (error) {
    return { status: 'error', error: error.message, http_status: null, resolved_url: pageSpec.text_url, text: '', rows: [] };
  }
}
async function downloadCandidate(context, pageSpec, row, variant, index) {
  let response;
  try {
    response = await context.request.get(variant.url, { headers: { 'User-Agent': UA, Referer: pageSpec.url, Accept: 'image/jpeg,image/webp,image/png,image/*,*/*;q=0.2' }, timeout: 90000, failOnStatusCode: false });
  } catch (error) {
  ...
  return { page_key: pageSpec.key, provider: pageSpec.provider, source_page: pageSpec.url, probe_url: variant.url, probe_kind: variant.kind, resolved_url: response.url() || variant.url, origin: row.origin, label: row.label || '', local_context: row.context || '', local, mime, bytes: bytes.length, sha256: sha(bytes), ...dimensions };
}

const control = await readJson(CONTROL);
assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-080', 'UC-080 repaired discovery scope drift');
assert(control.actor === 'Louis Gossett Jr.' && control.character === "Jeriba 'Jerry' Shigan" && control.side === 'still', 'UC-080 repaired discovery identity drift');
assert(control.selector_artifact?.artifact_id === 8668928932 && control.scope_artifact?.artifact_id === 8669004707, 'UC-080 repaired discovery custody drift');
assert(control.failed_checkpoints?.some(row => row.run_id === 30307804768 && row.artifact_id === 8669140420), 'UC-080 failed checkpoint custody missing');
assert(control.pages?.length === 8 && control.pages.filter(row => row.strict).length === 3, 'UC-080 repaired source denominator drift');
...
try {
  ...
  for (const pageSpec of control.pages) {
    const browserEvidence = await inspectBrowserPage(context, pageSpec);
    const textEvidence = await inspectTextEndpoint(context, pageSpec);
    const combinedText = [browserEvidence.body, browserEvidence.html, textEvidence?.text].filter(Boolean).join(' ');
    const missing = pageSpec.required_terms.filter(term => !norm(combinedText).includes(norm(term)));
    const transportOk = (browserEvidence.status === 'loaded' && browserEvidence.http_status >= 200 && browserEvidence.http_status < 400) || (textEvidence?.status === 'loaded' && textEvidence.http_status >= 200 && textEvidence.http_status < 400);
    page_evidence[pageSpec.key] = {
      browser: {
        status: browserEvidence.status,
        http_status: browserEvidence.http_status,
        title: browserEvidence.title,
        resolved_url: browserEvidence.resolved_url,
        error: browserEvidence.error || null,
        screenshot: browserEvidence.screenshot
      },
      text_endpoint: textEvidence ? {
        status: textEvidence.status,
        http_status: textEvidence.http_status,
        resolved_url: textEvidence.resolved_url,
        error: textEvidence.error || null,
        text_path: textEvidence.text_path || null,
        text_sha256: textEvidence.text_sha256 || null
      } : null,
      required_terms: pageSpec.required_terms,
      required_terms_missing: missing,
      body_text: String(browserEvidence.body || '').slice(0, 24000)
    };
    if (browserEvidence.screenshot) {
      try {
        const bytes = await readFile(join(OUT, browserEvidence.screenshot));
        page_screenshots.push({ page_key: pageSpec.key, provider: pageSpec.provider, path: browserEvidence.screenshot, sha256: sha(bytes), bytes: bytes.length });
      } catch {}
    }
    if (pageSpec.strict) {
      assert(transportOk, `${pageSpec.key} repaired page transport failed`);
      assert(missing.length === 0, `${pageSpec.key} repaired page evidence failed: ${missing.join(', ')}`);
    }

    const rows = [];
    const seen = new Set();
    for (const explicit of pageSpec.explicit_assets || []) addUrl(rows, seen, { url: explicit, label: 'explicit authorized asset', context: combinedText.slice(0, 6000), origin: 'explicit-control' }, pageSpec.url);
    for (const row of browserEvidence.rows || []) addUrl(rows, seen, row, browserEvidence.resolved_url || pageSpec.url);
    for (const row of textEvidence?.rows || []) addUrl(rows, seen, row, textEvidence.resolved_url || pageSpec.url);
    for (const match of String(browserEvidence.html || '').matchAll(/https?:\\?\/\\?\/[^"'<>\s]+?\.(?:jpe?g|png|webp)(?:\?[^"'<>\s]*)?/gi)) addUrl(rows, seen, { url: match[0], label: '', context: browserEvidence.title || '', origin: 'html-url' }, pageSpec.url);
  ...
  assert(candidates.length >= 2, `UC-080 repaired discovery produced only ${candidates.length} usable unique candidates`);
  ...
  console.log(`PASS — UC-080 repaired exact-role discovery retained ${candidates.length} candidate(s)`);
