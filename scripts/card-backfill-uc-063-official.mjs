...
try{
  ...
  for(const pinned of control.pinned_assets||[])addUrl(urlMap,pinned.url,'pinned-official',pinned.page_context||pinned.label||'official pinned asset');
  for(const row of extracted)addUrl(urlMap,row.value,row.kind,row.context);
  for(const match of html.replace(/\\u002F/gi,'/').replace(/\\\//g,'/').matchAll(/https?:[^"'<>\s)]+/g))addUrl(urlMap,match[0],'html-regex','page source');
  const urls=[...urlMap.values()].map(row=>({...row,source_score:scoreUrl(row)})).sort((a,b)=>b.source_score-a.source_score||a.url.localeCompare(b.url)).slice(0,Number(control.max_urls||40));
  const downloaded=[],seenHashes=new Set();let index=0;
  for(const row of urls){let response;try{response=await context.request.get(row.url,{headers:{'User-Agent':UA,Referer:control.source_page,Accept:'image/avif,image/webp,image/png,image/jpeg,*/*;q=0.2'},timeout:20000,failOnStatusCode:false})}catch(error){continue}if(!response.ok())continue;const bytes=Buffer.from(await response.body()),mime=signatureMime(bytes);if(bytes.length<8000||mime==='unknown')continue;const local=`candidates/${String(++index).padStart(3,'0')}-${slug(row.kinds[0]||'image')}.${extensionFor(mime)}`,path=join(OUT,local);await mkdir(dirname(path),{recursive:true});await writeFile(path,bytes);let dimensions;try{dimensions=identify(path)}catch{continue}if(dimensions.width<320||dimensions.height<240)continue;const hash=sha(bytes);if(seenHashes.has(hash))continue;seenHashes.add(hash);const distance=visualDistance(referencePath,path,index);downloaded.push({...row,local,mime,bytes:bytes.length,sha256:hash,...dimensions,resolved_url:response.url()||row.url,repository_matches:repository.get(hash)||[],reference_rmse:distance})}
