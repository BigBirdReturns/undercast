...
import { validateAmortizedWaveBinding } from "./lib/card-backfill-amortization.mjs";
...
async function findNamedFiles(root, name, out = []) {
  let entries = [];
  try { entries = await readdir(root, { withFileTypes: true }); } catch (error) { if (error.code === "ENOENT") return out; throw error; }
...
async function main() {
  ...
  const amortizationPlanPath = resolve(option("--amortization-plan", join(dirname(wavePath), "amortization-plan.json")));
  ...
  const performanceRoot = resolve(option("--performance-root", ".github/card-backfill/performance"));
  const sourceHead = option("--source-head");
  const mutationHead = option("--mutation-head");
  ...
  const amortizationPlan = await readJson(amortizationPlanPath);
  validateAmortizedWaveBinding(wave, amortizationPlan);
  ...
  for (const result of results) {
    ...
    if (result.amortization_plan_sha256 !== wave.amortization_plan_sha256 || result.cost_model_sha256 !== wave.cost_model_sha256) throw new Error(`wave result amortization drift ${result.batch_sha256}`);
    if (result.rediscovery !== false) throw new Error(`wave result rediscovery drift ${result.batch_sha256}`);
  ...
  const performance = await reducePerformanceObservations({ wave, resultsRoot, performanceRoot, now });
  const ledger = await validateStaging({ root: stagingRoot, permanentRoot });
  const receiptBase = {
    version: 1,
    lane: "card-backfill-source-v3-amortized-wave-reduction",
    reduced_at: now,
    source_head: sourceHead,
    mutation_head: mutationHead,
    wave_sha256: wave.wave_sha256,
    ...
    amortization_plan_sha256: wave.amortization_plan_sha256,
    input_cost_model_sha256: wave.cost_model_sha256 || null,
    counts: {
      ...
      performance_observations: performance.observation.observed_count,
    },
    reduced_batches: reduced,
    staging_ledger_sha256: ledger.ledger_sha256,
    performance_observation_sha256: performance.observation.observation_sha256,
    cost_model_sha256: performance.model.model_sha256,
    source_rediscovery_during_reduction: false,
    exact_mutation_head_required_before_push: true,
    canonical_mutation: false,
  };
  ...
  console.log(`PASS — reduced ${reduced.length} immutable wave result(s) into one amortized branch transaction`);
  ...
  console.log(`AMORTIZATION — observations=${reduction.counts.performance_observations} model=${reduction.cost_model_sha256}`);
}

main().catch((error) => {
  console.error(`card-backfill amortized wave reduce: ${error.message}`);
  process.exit(1);
});
