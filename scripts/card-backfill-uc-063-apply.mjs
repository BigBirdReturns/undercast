...
const CONTROL='.github/CARD-BACKFILL-UC-063-APPLY.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const DEST='data/review/card-backfill/UC-063';
...
const expectedFiles=['SHA256SUMS','card-crop-preview.jpg','duplicate-scan.json','manifest.json','review.json','review.md','source-page.png','uc-063-still-candidate.jpg'];

async function loadControl(){
  const control=await readJson(CONTROL);
  assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-063','UC-063 apply control scope drift');
  assert(control.actor==='Jim Carrey'&&control.character==='The Grinch'&&control.side==='still'&&control.reviewed_role==='second-desk','UC-063 apply authority drift');
  assert(control.render_artifact?.artifact_id===8635782398&&control.render_artifact?.head_sha==='b5c8de58ede68e6441b95dd3eb5eb24ea4d65414','UC-063 render artifact custody drift');
  ...
  assert(control.ruling?.identity==='expected-subject'&&control.ruling?.presentation==='character-depiction'&&control.ruling?.crop_ruling==='pass-east-focus'&&control.ruling?.canonical_mutation===false,'UC-063 apply ruling drift');
  return control;
}
...
async function materialize(){
  ...
  const files={manifest:'manifest.json',duplicates:'duplicate-scan.json',review:'review.json',reviewMd:'review.md',candidate:'uc-063-still-candidate.jpg',crop:'card-crop-preview.jpg',page:'source-page.png'};
  ...
  assert(sourceManifest.record_id==='UC-063'&&sourceManifest.actor==='Jim Carrey'&&sourceManifest.character==='The Grinch'&&sourceManifest.side==='still','source manifest identity drift');
  assert(sourceManifest.source?.provider==='Universal Pictures At Home'&&sourceManifest.source?.source_page==='https://www.universalpicturesathome.com/movies/dr-seuss-how-the-grinch-stole-christmas','source provenance drift');
  assert(String(sourceManifest.source?.gallery_context||'').includes('Extended Preview'),'official gallery context drift');
  ...
  for(const file of['uc-063-still-candidate.jpg','card-crop-preview.jpg','source-page.png'])await copyFile(join(SOURCE_ROOT,file),join(DEST,file));
  ...
  await writeFile(join(DEST,'review.md'),`# UC-063 reviewed Grinch still candidate\n\n- **Record:** UC-063\n- **Performer:** Jim Carrey\n- **Displayed role:** The Grinch\n- **Production:** How the Grinch Stole Christmas (2000)\n- **Source:** [Universal Pictures At Home](${manifest.source.source_page})\n- **Candidate:** \`${control.expected.candidate_sha256}\`\n- **Wall-crop preview:** \`${control.expected.crop_preview_sha256}\`\n- **Identity ruling:** expected subject\n- **Presentation ruling:** character depiction\n- **Crop ruling:** pass — east focus\n- **Canonical mutation:** none\n\n## Visual ruling\n\n${control.ruling.notes.map(note=>`- ${note}`).join('\n')}\n\nThe exact 2560×1440 official Universal gallery JPEG is retained unchanged. The east-focused crop preserves both eyes and the complete facial expression. This packet remains evidence-only pending independent canonical acceptance.\n`);
...
async function validate(){
  ...
  assert(JSON.stringify(names)===JSON.stringify([...expectedFiles].sort()),`UC-063 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.source?.provider==='Universal Pictures At Home'&&manifest.crop_preview?.gravity==='east','source or crop receipt drift');
  ...
  assert((await fileMeta(join(DEST,'uc-063-still-candidate.jpg'))).sha256===control.expected.candidate_sha256,'candidate bytes drift');
