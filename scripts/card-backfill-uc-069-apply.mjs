...
const CONTROL='.github/CARD-BACKFILL-UC-069-APPLY.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const DEST='data/review/card-backfill/UC-069';
...
const expectedFiles=['SHA256SUMS','card-crop-preview.jpg','duplicate-scan.json','manifest.json','review.json','review.md','source-page.png','uc-069-still-candidate.jpg'];

async function loadControl(){
  const control=await readJson(CONTROL);
  assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-069','UC-069 apply scope drift');
  assert(control.actor==='Danny DeVito'&&control.character==='The Penguin'&&control.side==='still'&&control.reviewed_role==='second-desk','UC-069 apply authority drift');
  assert(control.render_artifact?.artifact_id===8636506496&&control.render_artifact?.head_sha==='3eb06a2b3020d8461cef043af32f093d97a0e558','UC-069 render artifact custody drift');
  ...
  assert(control.ruling?.identity==='expected-subject'&&control.ruling?.presentation==='character-depiction'&&control.ruling?.crop_ruling==='pass-east-focus'&&control.ruling?.canonical_mutation===false,'UC-069 apply ruling drift');
  return control;
}
...
async function materialize(){
  ...
  const files={manifest:'manifest.json',duplicates:'duplicate-scan.json',review:'review.json',reviewMd:'review.md',candidate:'uc-069-still-candidate.jpg',crop:'card-crop-preview.jpg',page:'source-page.png'};
  const sourceMeta={};for(const[name,file]of Object.entries(files))sourceMeta[name]=await fileMeta(join(SOURCE_ROOT,file));
  ...
  assert(sourceManifest.record_id==='UC-069'&&sourceManifest.actor==='Danny DeVito'&&sourceManifest.character==='The Penguin'&&sourceManifest.side==='still','source manifest identity drift');
  assert(sourceManifest.source?.provider==='DC'&&sourceManifest.source?.source_page==='https://www.dc.com/movies/batman-returns-1992','source provenance drift');
  assert(sourceManifest.source?.source_key==='dc-gallery-03'&&sourceManifest.source?.gallery_label==='Batman Returns (1992) - GALLERY','source gallery boundary drift');
  assert(String(sourceManifest.source?.synopsis_evidence||'').includes('Penguin')&&String(sourceManifest.source?.synopsis_evidence||'').includes('Danny DeVito'),'source synopsis evidence drift');
  ...
  assert(sourceManifest.crop_preview?.sha256===control.expected.crop_preview_sha256&&sourceManifest.crop_preview?.width===control.expected.crop_width&&sourceManifest.crop_preview?.height===control.expected.crop_height&&sourceManifest.crop_preview?.gravity==='east','source crop receipt drift');
  assert(sourceManifest.rejected_visual_duplicates?.length===6,'visual duplicate receipt drift');
  ...
  assert(sourceReview.identity_ruling==='expected-subject'&&sourceReview.presentation_ruling==='character-depiction'&&sourceReview.crop_ruling==='pass-east-focus'&&sourceReview.canonical_mutation===false,'source review disposition drift');

  await rm(DEST,{recursive:true,force:true});await mkdir(DEST,{recursive:true});
  for(const file of['uc-069-still-candidate.jpg','card-crop-preview.jpg','source-page.png'])await copyFile(join(SOURCE_ROOT,file),join(DEST,file));
  ...
  await writeFile(join(DEST,'review.md'),`# UC-069 reviewed Penguin still candidate\n\n- **Record:** UC-069\n- **Performer:** Danny DeVito\n- **Displayed role:** The Penguin\n- **Production:** Batman Returns (1992)\n- **Source:** [Official DC movie page](${manifest.source.source_page})\n- **Candidate:** \`${control.expected.candidate_sha256}\`\n- **Wall-crop preview:** \`${control.expected.crop_preview_sha256}\`\n- **Identity ruling:** expected subject\n- **Presentation ruling:** character depiction\n- **Crop ruling:** pass — east focus\n- **Canonical mutation:** none\n\n## Visual ruling\n\n${control.ruling.notes.map(note=>`- ${note}`).join('\n')}\n\nThe exact unparameterized 1920×1080 official DC JPEG is retained unchanged. Six parameterized deliveries of the same composition remain recorded as rejected visual duplicates. The east-focused crop makes DeVito's Penguin the dominant subject while retaining Catwoman and the lair only as scene context. This packet remains evidence-only pending independent canonical acceptance.\n`);
...
async function validate(){
  const control=await loadControl();
  const names=(await readdir(DEST)).sort();assert(JSON.stringify(names)===JSON.stringify([...expectedFiles].sort()),`UC-069 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.source?.provider==='DC'&&manifest.crop_preview?.gravity==='east'&&manifest.rejected_visual_duplicates?.length===6,'source/crop/visual duplicate receipt drift');
  assert(review.identity_ruling==='expected-subject'&&review.presentation_ruling==='character-depiction'&&review.crop_ruling==='pass-east-focus'&&review.canonical_mutation===false,'review receipt drift');
  ...
  assert((await fileMeta(join(DEST,'uc-069-still-candidate.jpg'))).sha256===control.expected.candidate_sha256,'candidate bytes drift');
