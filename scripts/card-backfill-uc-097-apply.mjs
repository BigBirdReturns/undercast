...
const CONTROL = process.env.CONTROL || '.github/CARD-BACKFILL-UC-097-APPLY.json';
const SOURCE_ROOT = process.env.SOURCE_ROOT || '';
const DEST = process.env.DEST || 'data/review/card-backfill/UC-097';
...
async function loadControl() {
  ...
  assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-097', 'UC-097 apply scope drift');
  assert(control.actor === 'Dan Stevens' && control.character === 'The Beast' && control.production === 'Beauty and the Beast' && control.year === 2017 && control.side === 'still' && control.reviewed_role === 'second-desk', 'UC-097 apply authority drift');
  assert(control.render_artifact?.artifact_id === 8672735726 && control.render_artifact?.head_sha === '8d283acce33530defe75c6ed173f9d9185429f80', 'UC-097 render custody drift');
  for (const key of ['source_manifest_sha256','source_duplicate_scan_sha256','source_review_json_sha256','source_review_md_sha256','source_sums_sha256','exact_role_record_sha256','selected_source_sha256','disney_movies_page_sha256','disney_company_page_sha256','d23_costume_page_sha256','candidate_sha256','crop_preview_sha256']) {
    assert(/^[0-9a-f]{64}$/.test(control.expected?.[key] || ''), `missing expected ${key}`);
  }
  assert(control.expected?.packet_file_count === 12 && control.expected?.duplicate_item_count === 3 && control.expected?.duplicate_repository_hash_count === 2070, 'UC-097 packet denominator drift');
  assert(control.ruling?.identity === 'expected-subject' && control.ruling?.presentation === 'character-depiction' && control.ruling?.crop_ruling === 'pass-single-role-center-crop' && control.ruling?.canonical_mutation === false, 'UC-097 apply ruling drift');
  return control;
}
...
async function verifySourcePacket(control) {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-097 source packet file set drift: ${names.join(', ')}`);
  ...
  await verifyFile('beast-original.jpg', { sha256: control.expected.selected_source_sha256, mime: control.expected.selected_source_mime, bytes: control.expected.selected_source_bytes });
  await verifyFile('source-page-disney-movies-2017.png', { sha256: control.expected.disney_movies_page_sha256, mime: 'image/png', bytes: control.expected.disney_movies_page_bytes });
  await verifyFile('source-page-disney-company-announcement.png', { sha256: control.expected.disney_company_page_sha256, mime: 'image/png', bytes: control.expected.disney_company_page_bytes });
  await verifyFile('source-page-d23-beast-costume.png', { sha256: control.expected.d23_costume_page_sha256, mime: 'image/png', bytes: control.expected.d23_costume_page_bytes });
  await verifyFile('uc-097-still-candidate.jpg', { sha256: control.expected.candidate_sha256, mime: control.expected.candidate_mime, bytes: control.expected.candidate_bytes });
  ...
  assert(manifest.record_id === 'UC-097' && manifest.actor === 'Dan Stevens' && manifest.character === 'The Beast' && manifest.production === 'Beauty and the Beast' && manifest.year === 2017 && manifest.side === 'still', 'source manifest identity drift');
  assert(manifest.selected_source?.original?.sha256 === control.expected.selected_source_sha256 && manifest.selected_source?.source_page_screenshot?.sha256 === control.expected.disney_movies_page_sha256, 'source selection receipt drift');
  assert(manifest.role_custody?.record?.sha256 === control.expected.exact_role_record_sha256, 'source exact-role receipt drift');
  assert(manifest.role_custody?.release_and_cast?.page_screenshot?.sha256 === control.expected.disney_company_page_sha256, 'source Disney company receipt drift');
  assert(manifest.role_custody?.digital_costume?.page_screenshot?.sha256 === control.expected.d23_costume_page_sha256, 'source D23 costume receipt drift');
  ...
  assert(role.record_id === 'UC-097' && role.actor === 'Dan Stevens' && role.character === 'The Beast' && role.production === 'Beauty and the Beast' && role.year === 2017 && role.selected_still?.selected_image_sha256 === control.expected.selected_source_sha256 && role.canonical_mutation === false, 'exact role record drift');
...
async function validate() {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-097 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.record_id === 'UC-097' && manifest.actor === 'Dan Stevens' && manifest.character === 'The Beast' && manifest.production === 'Beauty and the Beast' && manifest.candidate?.sha256 === control.expected.candidate_sha256, 'permanent manifest identity drift');
  assert(manifest.selected_source?.original?.sha256 === control.expected.selected_source_sha256 && manifest.selected_source?.source_page_screenshot?.sha256 === control.expected.disney_movies_page_sha256, 'permanent source receipt drift');
  assert(manifest.role_custody?.record?.sha256 === control.expected.exact_role_record_sha256 && manifest.role_custody?.release_and_cast?.page_screenshot?.sha256 === control.expected.disney_company_page_sha256 && manifest.role_custody?.digital_costume?.page_screenshot?.sha256 === control.expected.d23_costume_page_sha256, 'permanent role custody drift');
  ...
  assert(role.record_id === 'UC-097' && role.selected_still?.selected_image_sha256 === control.expected.selected_source_sha256 && role.canonical_mutation === false, 'permanent exact-role record drift');
  ...
  assert((await receipt(join(DEST, 'beast-original.jpg'))).sha256 === control.expected.selected_source_sha256, 'permanent source bytes drift');
  assert((await receipt(join(DEST, 'uc-097-still-candidate.jpg'))).sha256 === control.expected.candidate_sha256, 'permanent candidate bytes drift');
  ...
  console.log(`VALID ${DEST}: exact reviewed Beast evidence packet; no canonical mutation`);
}
