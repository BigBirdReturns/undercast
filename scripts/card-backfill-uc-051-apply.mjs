...
const CONTROL='.github/CARD-BACKFILL-UC-051-APPLY.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const DEST='data/review/card-backfill/UC-051';
...
const expectedFiles=['SHA256SUMS','card-crop-preview.jpg','duplicate-scan.json','manifest.json','review.json','review.md','source-page.png','uc-051-still-candidate.jpg'];

async function loadControl(){
  const control=await readJson(CONTROL);
  assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-051','apply control scope drift');
  assert(control.actor==='Jeff Goldblum'&&control.character==='Brundlefly'&&control.side==='still'&&control.reviewed_role==='second-desk','apply authority drift');
  assert(control.render_artifact?.artifact_id===8634336171&&control.render_artifact?.head_sha==='d8a20ba4258b6ef2ae875b8c216502b07c99fbee','render artifact custody drift');
...
async function materialize(){
  ...
  const sourceFiles={manifest:'manifest.json',duplicates:'duplicate-scan.json',review:'review.json',reviewMd:'review.md',candidate:'uc-051-still-candidate.jpg',crop:'card-crop-preview.jpg',page:'source-page.png'};
  const sourceMeta={};
  ...
  assert(sourceManifest.record_id==='UC-051'&&sourceManifest.actor==='Jeff Goldblum'&&sourceManifest.character==='Brundlefly'&&sourceManifest.side==='still','source manifest identity drift');
  assert(sourceManifest.source?.provider==='Bloody Disgusting'&&sourceManifest.source?.source_key==='bloody-disgusting-effects'&&sourceManifest.source?.source_page==='https://bloody-disgusting.com/editorials/3545301/came-80s-oscar-winning-makeup-creature-effects-transformed-fly/','source provenance drift');
  assert(String(sourceManifest.source?.article_evidence||'').includes('seven transformation stages')&&String(sourceManifest.source?.article_evidence||'').includes('final creature'),'article evidence drift');
  ...
  for(const file of['uc-051-still-candidate.jpg','card-crop-preview.jpg','source-page.png'])await copyFile(join(SOURCE_ROOT,file),join(DEST,file));
...
async function validate(){
  ...
  assert(JSON.stringify(names)===JSON.stringify([...expectedFiles].sort()),`UC-051 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.source?.provider==='Bloody Disgusting'&&manifest.source?.source_page==='https://bloody-disgusting.com/editorials/3545301/came-80s-oscar-winning-makeup-creature-effects-transformed-fly/','source receipt drift');
  ...
  const candidate=await fileMeta(join(DEST,'uc-051-still-candidate.jpg'));
