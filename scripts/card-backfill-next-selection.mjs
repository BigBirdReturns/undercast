...
for(const item of audit.items||[]){
  if(item.scope!=='sitewide'||item.status!=='absent'||!['still','portrait'].includes(item.side)||complete.has(item.wall_id))continue;
  const r=byId.get(item.wall_id);if(!r)continue;
  rows.push({wall_id:item.wall_id,side:item.side,audit_id:item.id,actor:r.actor,character:r.character,production:r.production,universe:r.universe,years:r.years||'',expected_subject:item.expected_subject,references:r.references||[]});
}
