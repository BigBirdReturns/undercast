...
try{
  for(const entry of await readdir('data/review/card-backfill',{withFileTypes:true})){
    if(!entry.isDirectory())continue;
    try{
      const review=await readJson(join('data/review/card-backfill',entry.name,'review.json'));
      const id=review?.record_id||review?.id;
...
for(const item of audit.items||[]){
  if(item.scope!=='sitewide'||item.status!=='absent'||!['still','portrait'].includes(item.side)||completed.has(item.wall_id))continue;
  const record=specimenById.get(item.wall_id);if(!record)continue;
  rows.push({wall_id:item.wall_id,side:item.side,audit_id:item.id,actor:record.actor,character:record.character,production:record.production,universe:record.universe,years:record.years||'',canonical_link:record.link||'',expected_subject:item.expected_subject,source_fetched_at:item.source_fetched_at||null,risk_codes:item.risk_codes||[],references:record.references||[],current_asset:record[item.side]||null});
}
