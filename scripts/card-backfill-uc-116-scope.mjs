...
const CONTROL = '.github/CARD-BACKFILL-UC-116.json';
const SPECIMENS = 'data/specimens.json';
const AUDIT = 'data/MEDIA-AUDIT.json';
const SOURCES = 'data/SOURCES.json';
const OUT = process.env.OUT || '/tmp/card-backfill-uc-116-scope';
...
assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-116', 'UC-116 control scope drift');
assert(control.actor === 'June Foray' && control.character === 'Rocky, Natasha & Granny' && control.production === 'Rocky & Bullwinkle / Looney Tunes' && control.years === '1959' && control.side === 'still', 'UC-116 selector identity drift');
assert(control.selector_artifact?.artifact_id === 8677539993 && control.selector_artifact?.head_sha === '240edfbbf4974e6d8b4a19a2b40a4bb2e9571933', 'UC-116 selector custody drift');
assert(control.scope_contract?.exact_three_role_voice_composite_required === true && control.scope_contract?.required_roles?.length === 3 && control.scope_contract?.all_three_panels_required === true && control.scope_contract?.canonical_mutation === false, 'UC-116 scope contract drift');
...
assert(specimen, 'UC-116 specimen missing');
assert(specimen.actor === control.actor, `UC-116 specimen actor drift: ${specimen.actor}`);
assert(specimen.character === control.character, `UC-116 specimen character drift: ${specimen.character}`);
assert(specimen.production === control.production, `UC-116 specimen production drift: ${specimen.production}`);
assert(specimen.universe === control.universe, `UC-116 specimen universe drift: ${specimen.universe}`);
assert(specimen.years === control.years, `UC-116 specimen years drift: ${specimen.years}`);
assert(!specimen.still, 'UC-116 specimen already has a canonical still');
assert(audit, 'UC-116 audit row missing');
assert(audit.wall_id === control.record_id && audit.side === control.side, 'UC-116 audit identity drift');
assert(audit.scope === 'sitewide' && audit.status === 'absent' && !audit.asset, 'UC-116 audit absence drift');
assert(audit.expected_subject === control.expected_subject, `UC-116 expected-subject drift: ${audit.expected_subject}`);
assert(!source?.still, 'UC-116 source ledger already has a canonical still');
...
const filedRoleSignals = Object.fromEntries(requiredRoles.map(role => [
  ...
  normalizedFiledRoles.some(value => {
    if (role === 'Rocky the Flying Squirrel') return value.includes('rocky');
    if (role === 'Natasha Fatale') return value.includes('natasha');
    if (role === 'Granny') return value.includes('granny');
    return false;
  })
]));
...
const scope = {
  ...
  invariants: {
    ...
    june_foray_voice_binding_required_for_each_role: control.scope_contract.june_foray_voice_binding_required_for_each_role,
    original_foray_performance_era_required: control.scope_contract.original_foray_performance_era_required,
    all_three_panels_required: control.scope_contract.all_three_panels_required,
    bullwinkle_for_rocky_forbidden: control.scope_contract.reject_bullwinkle_for_rocky,
    boris_for_natasha_forbidden: control.scope_contract.reject_boris_for_natasha,
    tweety_or_sylvester_for_granny_forbidden: control.scope_contract.reject_tweety_or_sylvester_for_granny,
    later_reboot_without_foray_custody_forbidden: control.scope_contract.reject_later_reboots_without_foray_custody,
...
console.log(`PASS — exact UC-116 canonical scope extracted: ${declaredPerformances.length} declared performance(s), ${references.length} reference(s)`);
