...
import { buildSourcePolicyV3Estate, CARD_BACKFILL_SOURCE_POLICY_V3 } from "./lib/card-backfill-source-policy-v3.mjs";
...
async function main() {
  ...
  const out = resolve(option("--out", ".card-backfill-source-v3"));
  ...
  const retryEstate = buildSourcePolicyV3Estate({
    estate: sourceEstate,
    attemptIndex,
  ...
  const exclusionState = {
    ...
    composite_queue_sha256: sha256(canonicalJson(retryEstate.composite_required || [])),
  };
  ...
  batch.source_policy = CARD_BACKFILL_SOURCE_POLICY_V3;
  batch.source_policy_version = CARD_BACKFILL_SOURCE_POLICY_V3.version;
  ...
  await writeJson(join(out, "cohorts.json"), { version: 1, campaign_id: retryEstate.campaign_id, estate_sha256: retryEstate.estate_sha256, source_policy: CARD_BACKFILL_SOURCE_POLICY_V3, counts: retryEstate.counts, cohorts: retryEstate.cohorts });
  ...
  await writeJson(join(out, "composite-queue.json"), { version: 1, lane: "card-backfill-source-policy-v3-composite-queue", campaign_id: retryEstate.campaign_id, count: retryEstate.composite_required.length, items: retryEstate.composite_required, canonical_mutation: false });
  ...
  await writeJson(join(out, "shards.json"), { version: 1, campaign_id: batch.campaign_id, batch_sha256: batch.batch_sha256, source_policy_version: 3, workers: shards.length, matrix });
  for (const row of batch.obligations) {
    ...
    await writeJson(join(out, "batch-scopes", `${scope.record_id}-${scope.side}.json`), { ...scope, source_policy: CARD_BACKFILL_SOURCE_POLICY_V3, source_policy_version: 3 });
  }
  await writeFile(join(out, "summary.txt"), [
    ...
    `composite_required=${retryEstate.counts.composite_required}`,
  ...
  console.log(`PASS — source policy v3 admits ${retryEstate.counts.ready} single-subject still obligation(s) across ${retryEstate.counts.cohorts} cohort(s)`);
  console.log(`COMPOSITE — ${retryEstate.counts.composite_required} multi-subject obligation(s) retained for a separate deterministic composite lane`);
