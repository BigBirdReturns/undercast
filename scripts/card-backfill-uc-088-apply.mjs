...
const CONTROL = process.env.CONTROL || '.github/CARD-BACKFILL-UC-088-APPLY.json';
const SOURCE_ROOT = process.env.SOURCE_ROOT || '';
const DEST = process.env.DEST || 'data/review/card-backfill/UC-088';
...
async function loadControl() {
  ...
  assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-088', 'UC-088 apply scope drift');
  assert(control.actor === 'Michael Chiklis' && control.character === 'The Thing' && control.production === 'Fantastic Four' && control.year === 2005 && control.side === 'still' && control.reviewed_role === 'second-desk', 'UC-088 apply authority drift');
  assert(control.render_artifact?.artifact_id === 8672213650 && control.render_artifact?.head_sha === '4e54f469cff6a13987dfe7fe4793bd06798f2cf7', 'UC-088 render custody drift');
  for (const key of ['source_manifest_sha256','source_duplicate_scan_sha256','source_review_json_sha256','source_review_md_sha256','source_sums_sha256','exact_role_record_sha256','selected_source_sha256','movieinsider_page_sha256','afi_page_sha256','suit_page_sha256','scene_page_sha256','candidate_sha256','crop_preview_sha256']) {
    assert(/^[0-9a-f]{64}$/.test(control.expected?.[key] || ''), `missing expected ${key}`);
  }
  assert(control.expected?.packet_file_count === 13 && control.expected?.duplicate_item_count === 3 && control.expected?.duplicate_repository_hash_count === 2070, 'UC-088 packet denominator drift');
  assert(control.ruling?.identity === 'expected-subject' && control.ruling?.presentation === 'character-depiction' && control.ruling?.crop_ruling === 'pass-single-role-center-crop' && control.ruling?.canonical_mutation === false, 'UC-088 apply ruling drift');
  return control;
}
...
async function verifySourcePacket(control) {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-088 source packet file set drift: ${names.join(', ')}`);
  ...
  await verifyFile('thing-original.jpg', { sha256: control.expected.selected_source_sha256, mime: control.expected.selected_source_mime, bytes: control.expected.selected_source_bytes });
  await verifyFile('source-page-movieinsider-659.png', { sha256: control.expected.movieinsider_page_sha256, mime: 'image/png', bytes: control.expected.movieinsider_page_bytes });
  await verifyFile('source-page-afi-production-cast.png', { sha256: control.expected.afi_page_sha256, mime: 'image/png', bytes: control.expected.afi_page_bytes });
  await verifyFile('source-page-superherohype-full-costume.png', { sha256: control.expected.suit_page_sha256, mime: 'image/png', bytes: control.expected.suit_page_bytes });
  await verifyFile('source-page-superherohype-pigeon.png', { sha256: control.expected.scene_page_sha256, mime: 'image/png', bytes: control.expected.scene_page_bytes });
  await verifyFile('uc-088-still-candidate.jpg', { sha256: control.expected.candidate_sha256, mime: control.expected.candidate_mime, bytes: control.expected.candidate_bytes });
  ...
  assert(manifest.record_id === 'UC-088' && manifest.actor === 'Michael Chiklis' && manifest.character === 'The Thing' && manifest.production === 'Fantastic Four' && manifest.year === 2005 && manifest.side === 'still', 'source manifest identity drift');
  assert(manifest.selected_source?.original?.sha256 === control.expected.selected_source_sha256 && manifest.selected_source?.source_page_screenshot?.sha256 === control.expected.movieinsider_page_sha256, 'source selection receipt drift');
  assert(manifest.role_custody?.record?.sha256 === control.expected.exact_role_record_sha256, 'source exact-role receipt drift');
  assert(manifest.role_custody?.production_cast?.page_screenshot?.sha256 === control.expected.afi_page_sha256, 'source AFI receipt drift');
  assert(manifest.role_custody?.practical_suit?.page_screenshot?.sha256 === control.expected.suit_page_sha256, 'source practical-suit receipt drift');
  assert(manifest.role_custody?.selected_scene?.page_screenshot?.sha256 === control.expected.scene_page_sha256, 'source selected-scene receipt drift');
  ...
  assert(review.source_sha256 === control.expected.selected_source_sha256 && review.role_record_sha256 === control.expected.exact_role_record_sha256, 'source review source receipt drift');
  ...
  assert(role.record_id === 'UC-088' && role.actor === 'Michael Chiklis' && role.character === 'The Thing' && role.production === 'Fantastic Four' && role.year === 2005 && role.selected_still?.selected_image_sha256 === control.expected.selected_source_sha256 && role.canonical_mutation === false, 'exact role record drift');
...
async function validate() {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-088 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.record_id === 'UC-088' && manifest.actor === 'Michael Chiklis' && manifest.character === 'The Thing' && manifest.production === 'Fantastic Four' && manifest.candidate?.sha256 === control.expected.candidate_sha256, 'permanent manifest identity drift');
  assert(manifest.selected_source?.original?.sha256 === control.expected.selected_source_sha256 && manifest.selected_source?.source_page_screenshot?.sha256 === control.expected.movieinsider_page_sha256, 'permanent source receipt drift');
  assert(manifest.role_custody?.record?.sha256 === control.expected.exact_role_record_sha256 && manifest.role_custody?.production_cast?.page_screenshot?.sha256 === control.expected.afi_page_sha256 && manifest.role_custody?.practical_suit?.page_screenshot?.sha256 === control.expected.suit_page_sha256 && manifest.role_custody?.selected_scene?.page_screenshot?.sha256 === control.expected.scene_page_sha256, 'permanent role custody drift');
  assert(manifest.crop_preview?.sha256 === control.expected.crop_preview_sha256 && review.crop_preview_sha256 === control.expected.crop_preview_sha256, 'permanent crop receipt drift');
  assert(review.identity_ruling === 'expected-subject' && review.presentation_ruling === 'character-depiction' && review.crop_ruling === 'pass-single-role-center-crop' && review.canonical_mutation === false, 'permanent review ruling drift');
  assert(role.record_id === 'UC-088' && role.selected_still?.selected_image_sha256 === control.expected.selected_source_sha256 && role.canonical_mutation === false, 'permanent exact-role record drift');
  assert(duplicates.status === 'pass' && duplicates.repository_hash_count === 2070 && duplicates.items?.length === 3 && duplicates.items.every(item => item.matches?.length === 0), 'permanent duplicate receipt drift');
  assert((await receipt(join(DEST, 'thing-original.jpg'))).sha256 === control.expected.selected_source_sha256, 'permanent source bytes drift');
  assert((await receipt(join(DEST, 'uc-088-still-candidate.jpg'))).sha256 === control.expected.candidate_sha256, 'permanent candidate bytes drift');
  assert((await receipt(join(DEST, 'card-crop-preview.jpg'))).sha256 === control.expected.crop_preview_sha256, 'permanent crop bytes drift');
  for (const file of names) assert((await stat(join(DEST, file))).isFile(), `${file} is not a regular file`);
  console.log(`VALID ${DEST}: exact reviewed Thing evidence packet; no canonical mutation`);
}
