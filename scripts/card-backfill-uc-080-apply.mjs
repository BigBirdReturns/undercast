...
async function loadControl() {
  ...
  assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-080', 'UC-080 apply scope drift');
  assert(control.actor === 'Louis Gossett Jr.' && control.character === "Jeriba 'Jerry' Shigan" && control.side === 'still' && control.reviewed_role === 'second-desk', 'UC-080 apply authority drift');
  assert(control.render_artifact?.artifact_id === 8669650651 && control.render_artifact?.head_sha === '3472b559c639f07d6835500c27dcdc1ed2245409', 'UC-080 render custody drift');
  for (const key of ['source_manifest_sha256','source_duplicate_scan_sha256','source_review_json_sha256','source_review_md_sha256','source_sums_sha256','selected_source_sha256','historic_press_sha256','historic_product_sha256','apple_role_sha256','purepeople_role_sha256','moria_page_sha256','purepeople_page_sha256','candidate_sha256','crop_preview_sha256']) {
    assert(/^[0-9a-f]{64}$/.test(control.expected?.[key] || ''), `missing expected ${key}`);
  }
  assert(control.expected?.packet_file_count === 14 && control.expected?.duplicate_item_count === 4 && control.expected?.duplicate_repository_hash_count === 2070, 'UC-080 packet denominator drift');
  assert(control.ruling?.identity === 'expected-subject' && control.ruling?.presentation === 'character-depiction' && control.ruling?.crop_ruling === 'pass-single-role-west-crop' && control.ruling?.canonical_mutation === false, 'UC-080 apply ruling drift');
  return control;
}
...
async function verifySourcePacket(control) {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-080 source packet file set drift: ${names.join(', ')}`);
  ...
  await verifyFile('jeriba-original.jpg', { sha256: control.expected.selected_source_sha256, mime: control.expected.selected_source_mime, bytes: control.expected.selected_source_bytes });
  await verifyFile('historic-images-press-photo.webp', { sha256: control.expected.historic_press_sha256, mime: control.expected.historic_press_mime, bytes: control.expected.historic_press_bytes });
  await verifyFile('historic-images-product.txt', { sha256: control.expected.historic_product_sha256, bytes: control.expected.historic_product_bytes });
  await verifyFile('apple-tv-role.txt', { sha256: control.expected.apple_role_sha256, bytes: control.expected.apple_role_bytes });
  await verifyFile('purepeople-role.txt', { sha256: control.expected.purepeople_role_sha256, bytes: control.expected.purepeople_role_bytes });
  await verifyFile('source-page-moria.png', { sha256: control.expected.moria_page_sha256, mime: 'image/png', bytes: control.expected.moria_page_bytes });
  await verifyFile('source-page-purepeople.png', { sha256: control.expected.purepeople_page_sha256, mime: 'image/png', bytes: control.expected.purepeople_page_bytes });
  await verifyFile('uc-080-still-candidate.jpg', { sha256: control.expected.candidate_sha256, mime: control.expected.candidate_mime, bytes: control.expected.candidate_bytes });
  ...
  assert(manifest.record_id === 'UC-080' && manifest.actor === 'Louis Gossett Jr.' && manifest.character === "Jeriba 'Jerry' Shigan" && manifest.side === 'still', 'source manifest identity drift');
  assert(manifest.selected_source?.original?.sha256 === control.expected.selected_source_sha256, 'source original receipt drift');
  assert(manifest.corroboration?.historic_images?.press_photo?.sha256 === control.expected.historic_press_sha256, 'source Historic Images receipt drift');
  assert(manifest.corroboration?.historic_images?.product_record?.sha256 === control.expected.historic_product_sha256, 'source Historic product receipt drift');
  assert(manifest.corroboration?.apple_tv?.page_record?.sha256 === control.expected.apple_role_sha256, 'source Apple role receipt drift');
  assert(manifest.corroboration?.purepeople?.page_record?.sha256 === control.expected.purepeople_role_sha256 && manifest.corroboration?.purepeople?.page_screenshot?.sha256 === control.expected.purepeople_page_sha256, 'source Purepeople receipt drift');
  assert(manifest.selected_source?.source_page_screenshot?.sha256 === control.expected.moria_page_sha256, 'source Moria page receipt drift');
  ...
  assert(review.source_sha256 === control.expected.selected_source_sha256 && review.corroborating_press_photo_sha256 === control.expected.historic_press_sha256, 'source review source receipt drift');
...
async function materialize() {
  ...
  for (const hash of [control.expected.selected_source_sha256, control.expected.historic_press_sha256, control.expected.candidate_sha256, control.expected.crop_preview_sha256]) {
...
async function validate() {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-080 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.record_id === 'UC-080' && manifest.actor === 'Louis Gossett Jr.' && manifest.candidate?.sha256 === control.expected.candidate_sha256 && review.candidate_sha256 === control.expected.candidate_sha256, 'candidate receipt drift');
  ...
  assert(manifest.corroboration?.historic_images?.press_photo?.sha256 === control.expected.historic_press_sha256 && review.corroborating_press_photo_sha256 === control.expected.historic_press_sha256, 'Historic Images receipt drift');
  ...
  assert(duplicates.status === 'pass' && duplicates.repository_hash_count === 2070 && (duplicates.items || []).length === 4 && duplicates.items.every(item => item.matches?.length === 0), 'duplicate receipt drift');
  assert((await receipt(join(DEST, 'jeriba-original.jpg'))).sha256 === control.expected.selected_source_sha256, 'selected source bytes drift');
  assert((await receipt(join(DEST, 'uc-080-still-candidate.jpg'))).sha256 === control.expected.candidate_sha256, 'candidate bytes drift');
  ...
  console.log(`VALID ${DEST}: exact reviewed Jeriba evidence packet; no canonical mutation`);
}
