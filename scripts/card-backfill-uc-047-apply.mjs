...
const CONTROL='.github/CARD-BACKFILL-UC-047-APPLY.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const DEST='data/review/card-backfill/UC-047';
...
const expectedFiles=['SHA256SUMS','card-crop-preview.jpg','duplicate-scan.json','manifest.json','review.json','review.md','source-page.png','uc-047-still-candidate.jpg'];

async function loadControl(){
  const control=await readJson(CONTROL);
  assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-047','apply control scope drift');
  assert(control.actor==='Lon Chaney Jr.'&&control.character==='The Wolf Man'&&control.side==='still'&&control.reviewed_role==='second-desk','apply authority drift');
  assert(control.gather_artifact?.artifact_id===8634125583&&control.gather_artifact?.head_sha==='b52c17938243518106aeb3b4ace49b9e900e6ffd','gather artifact custody drift');
...
async function materialize(){
  ...
  const sourceFiles={manifest:'manifest.json',duplicates:'duplicate-scan.json',review:'review.json',reviewMd:'review.md',candidate:'uc-047-still-candidate.jpg',crop:'card-crop-preview.jpg',page:'source-page.png'};
  const sourceMeta={};
  ...
  assert(sourceManifest.record_id==='UC-047'&&sourceManifest.actor==='Lon Chaney Jr.'&&sourceManifest.character==='The Wolf Man'&&sourceManifest.side==='still','source manifest identity drift');
  assert(sourceManifest.source?.provider==='Wikimedia Commons'&&sourceManifest.source?.file_title==='File:Face detail, from- The Wolf Man lobby card (cropped).jpg','source provenance drift');
  assert(sourceManifest.source?.license==='Public domain'&&String(sourceManifest.source?.author||'').includes('Universal Studios'),'source rights metadata drift');
  ...
  assert(sourceManifest.crop_preview?.sha256===control.expected.crop_preview_sha256&&sourceManifest.crop_preview?.width===control.expected.crop_width&&sourceManifest.crop_preview?.height===control.expected.crop_height&&sourceManifest.crop_preview?.object_position_y===0.6,'source crop receipt drift');
  ...
  assert(sourceReview.identity_ruling==='pending'&&sourceReview.presentation_ruling==='pending'&&sourceReview.crop_ruling==='pending'&&sourceReview.canonical_mutation===false,'source pre-review disposition drift');

  await rm(DEST,{recursive:true,force:true});
  await mkdir(DEST,{recursive:true});
  for(const file of['uc-047-still-candidate.jpg','card-crop-preview.jpg','source-page.png'])await copyFile(join(SOURCE_ROOT,file),join(DEST,file));

  const duplicateScan={...sourceDuplicate,reviewed_at:control.reviewed_at,reviewed_by:control.reviewed_by,status:'pass',semantics:'Exact SHA-256 comparison against the canonical media manifest and present image tree at gather time. Evidence-packet files are not canonical media bindings.'};
  await writeJson(join(DEST,'duplicate-scan.json'),duplicateScan);
  const manifest={...sourceManifest,reviewed_at:control.reviewed_at,reviewed_by:control.reviewed_by,reviewed_role:control.reviewed_role,custody:{gather_artifact:control.gather_artifact,gather_control_sha256:sourceManifest.control_sha256,apply_control_sha256:sha(await readFile(CONTROL)),source_manifest_sha256:control.expected.source_manifest_sha256},duplicate_scan:{path:'duplicate-scan.json',repository_hash_count:duplicateScan.repository_hash_count,status:'pass'},exact_subject_review:{identity:control.ruling.identity,presentation:control.ruling.presentation,crop_ruling:control.ruling.crop_ruling,notes:control.ruling.notes},disposition:control.ruling.candidate_disposition,canonical_mutation:false};
...
async function validate(){
  ...
  assert(JSON.stringify(names)===JSON.stringify([...expectedFiles].sort()),`UC-047 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.source?.provider==='Wikimedia Commons'&&manifest.source?.source_page==='https://commons.wikimedia.org/wiki/File:Face_detail,_from-_The_Wolf_Man_lobby_card_(cropped).jpg'&&manifest.source?.license==='Public domain','source receipt drift');
  ...
  const candidate=await fileMeta(join(DEST,'uc-047-still-candidate.jpg'));
