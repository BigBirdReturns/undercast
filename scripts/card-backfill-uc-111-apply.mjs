...
const CONTROL = process.env.CONTROL || '.github/CARD-BACKFILL-UC-111-APPLY.json';
const SOURCE_ROOT = process.env.SOURCE_ROOT || '';
const DEST = process.env.DEST || 'data/review/card-backfill/UC-111';
...
const expectedFiles = [
  ...
  'manifest.json',
  'oompa-loompas-original.jpg',
  'review.json',
  'review.md',
  'source-page-afi-production-record.png',
  'source-page-chocolate-river-collective-frame.png',
  'source-page-warner-production-notes.png',
  'uc-111-still-candidate.jpg'
];

async function loadControl() {
  const control = await readJson(CONTROL);
  assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-111', 'UC-111 apply scope drift');
  assert(control.actor === 'Deep Roy' && control.character === 'The Oompa Loompas' && control.production === 'Charlie and the Chocolate Factory' && control.year === 2005 && control.side === 'still' && control.reviewed_role === 'second-desk', 'UC-111 apply authority drift');
  assert(control.render_artifact?.artifact_id === 8676276221 && control.render_artifact?.head_sha === 'fab76f6cf1c9a4c0ee201390de47efdcfcda973c', 'UC-111 render custody drift');
  for (const key of ['source_manifest_sha256','source_duplicate_scan_sha256','source_review_json_sha256','source_review_md_sha256','source_sums_sha256','exact_role_record_sha256','selected_source_sha256','frame_page_sha256','mechanism_page_sha256','production_page_sha256','candidate_sha256','crop_preview_sha256']) {
    assert(/^[0-9a-f]{64}$/.test(control.expected?.[key] || ''), `missing expected ${key}`);
  }
  assert(control.expected?.packet_file_count === 12 && control.expected?.duplicate_item_count === 3 && control.expected?.duplicate_repository_hash_count === 2070, 'UC-111 packet denominator drift');
  assert(control.ruling?.identity === 'expected-subject' && control.ruling?.presentation === 'collective-character-depiction' && control.ruling?.crop_ruling === 'pass-collective-east-zoom-crop' && control.ruling?.multiplicity_ruling === 'pass-many-body-single-performer-mechanism' && control.ruling?.canonical_mutation === false, 'UC-111 apply ruling drift');
  return control;
}
...
async function verifySourcePacket(control) {
  ...
  assert(JSON.stringify(names) === JSON.stringify([...expectedFiles].sort()), `UC-111 source packet file set drift: ${names.join(', ')}`);
  ...
  await verifyFile('oompa-loompas-original.jpg', { sha256: control.expected.selected_source_sha256, mime: control.expected.selected_source_mime, bytes: control.expected.selected_source_bytes });
  await verifyFile('source-page-chocolate-river-collective-frame.png', { sha256: control.expected.frame_page_sha256, mime: 'image/png', bytes: control.expected.frame_page_bytes });
  await verifyFile('source-page-warner-production-notes.png', { sha256: control.expected.mechanism_page_sha256, mime: 'image/png', bytes: control.expected.mechanism_page_bytes });
  await verifyFile('source-page-afi-production-record.png', { sha256: control.expected.production_page_sha256, mime: 'image/png', bytes: control.expected.production_page_bytes });
  await verifyFile('uc-111-still-candidate.jpg', { sha256: control.expected.candidate_sha256, mime: control.expected.candidate_mime, bytes: control.expected.candidate_bytes });
  ...
  assert(manifest.record_id === 'UC-111' && manifest.actor === 'Deep Roy' && manifest.character === 'The Oompa Loompas' && manifest.production === 'Charlie and the Chocolate Factory' && manifest.year === 2005 && manifest.side === 'still', 'source manifest identity drift');
  ...
  assert(manifest.role_custody?.performance_mechanism?.page_screenshot?.sha256 === control.expected.mechanism_page_sha256, 'source mechanism receipt drift');
  ...
  assert(manifest.role_custody?.multiplicity_boundary?.collective_display_required === true && manifest.role_custody?.multiplicity_boundary?.visible_repeated_bodies_required === true && manifest.role_custody?.multiplicity_boundary?.single_isolated_body_insufficient === true, 'source multiplicity boundary drift');
  ...
  assert(review.identity_ruling === control.ruling.identity && review.presentation_ruling === control.ruling.presentation && review.crop_ruling === control.ruling.crop_ruling && review.multiplicity_ruling === control.ruling.multiplicity_ruling && review.canonical_mutation === false, 'source review ruling drift');
  assert(role.record_id === 'UC-111' && role.actor === 'Deep Roy' && role.character === 'The Oompa Loompas' && role.year === 2005 && role.collective_frame?.selected_image_sha256 === control.expected.selected_source_sha256 && role.multiplicity_boundary?.single_isolated_body_insufficient === true && role.canonical_mutation === false, 'exact role record drift');
...
async function materialize() {
  ...
  const manifest = {
    ...
    exact_subject_review: { identity: control.ruling.identity, presentation: control.ruling.presentation, crop_ruling: control.ruling.crop_ruling, multiplicity_ruling: control.ruling.multiplicity_ruling, notes: control.ruling.notes },
  ...
  const review = {
    ...
    multiplicity_ruling: control.ruling.multiplicity_ruling,
    canonical_mutation: false,
    disposition: control.ruling.candidate_disposition,
  ...
  console.log('collective many-body mechanism preserved');
...
async function validate() {
  ...
  assert(manifest.record_id === 'UC-111' && manifest.actor === 'Deep Roy' && manifest.character === 'The Oompa Loompas' && manifest.production === 'Charlie and the Chocolate Factory' && manifest.year === 2005 && manifest.side === 'still', 'permanent manifest identity drift');
  ...
  assert(manifest.role_custody?.record?.sha256 === control.expected.exact_role_record_sha256 && manifest.role_custody?.multiplicity_boundary?.collective_display_required === true && manifest.role_custody?.multiplicity_boundary?.single_isolated_body_insufficient === true, 'permanent role or multiplicity boundary drift');
  assert(manifest.exact_subject_review?.identity === control.ruling.identity && manifest.exact_subject_review?.presentation === control.ruling.presentation && manifest.exact_subject_review?.crop_ruling === control.ruling.crop_ruling && manifest.exact_subject_review?.multiplicity_ruling === control.ruling.multiplicity_ruling && manifest.canonical_mutation === false, 'permanent review boundary drift');
  assert(review.identity_ruling === control.ruling.identity && review.presentation_ruling === control.ruling.presentation && review.crop_ruling === control.ruling.crop_ruling && review.multiplicity_ruling === control.ruling.multiplicity_ruling && review.canonical_mutation === false, 'permanent review drift');
  assert(role.multiplicity_boundary?.collective_display_required === true && role.multiplicity_boundary?.visible_repeated_bodies_required === true && role.multiplicity_boundary?.single_isolated_body_insufficient === true && role.canonical_mutation === false, 'permanent exact-role multiplicity drift');
