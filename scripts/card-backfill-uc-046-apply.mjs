...
const CONTROL='.github/CARD-BACKFILL-UC-046-APPLY.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const DEST='data/review/card-backfill/UC-046';
...
const expectedFiles=['SHA256SUMS','card-crop-preview.jpg','duplicate-scan.json','manifest.json','review.json','review.md','source-page.png','uc-046-still-candidate.jpg'];

async function loadControl(){
  const control=await readJson(CONTROL);
  assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-046','apply control scope drift');
  assert(control.actor==='Lon Chaney'&&control.character==='The Phantom'&&control.side==='still'&&control.reviewed_role==='second-desk','apply authority drift');
  assert(control.gather_artifact?.artifact_id===8633924238&&control.gather_artifact?.head_sha==='b45f11a9bd4dd85c656e67c68cb6986d0a6161e5','gather artifact custody drift');
...
async function materialize(){
  ...
  const sourceFiles={manifest:'manifest.json',duplicates:'duplicate-scan.json',review:'review.json',reviewMd:'review.md',candidate:'uc-046-still-candidate.jpg',crop:'card-crop-preview.jpg',page:'source-page.png'};
  const sourceMeta={};
  ...
  assert(sourceManifest.record_id==='UC-046'&&sourceManifest.actor==='Lon Chaney'&&sourceManifest.character==='The Phantom'&&sourceManifest.side==='still','source manifest identity drift');
  assert(sourceManifest.source?.provider==='Wikimedia Commons'&&sourceManifest.source?.file_title==='File:Lon Chaney as The Phantom.jpg','source provenance drift');
  assert(sourceManifest.source?.license==='Public domain'&&sourceManifest.source?.author==='Universal Pictures','source rights metadata drift');
  ...
  for(const file of['uc-046-still-candidate.jpg','card-crop-preview.jpg','source-page.png'])await copyFile(join(SOURCE_ROOT,file),join(DEST,file));
...
async function validate(){
  ...
  assert(JSON.stringify(names)===JSON.stringify([...expectedFiles].sort()),`UC-046 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.source?.provider==='Wikimedia Commons'&&manifest.source?.source_page==='https://commons.wikimedia.org/wiki/File:Lon_Chaney_as_The_Phantom.jpg'&&manifest.source?.license==='Public domain','source receipt drift');
  ...
  const candidate=await fileMeta(join(DEST,'uc-046-still-candidate.jpg'));
