...
OUTPUT = Path("scripts/.card-backfill-uc-176-discover-afi-dom-run.mjs")
...
new = """    const title = await page.title();
    ...
    const samePageDomText = source.key === 'afi-catalog'
      ? await page.locator('body').textContent().catch(() => '')
      : '';
    const body = source.key === 'afi-catalog'
...
"""
if text.count(old) != 1:
    raise SystemExit("UC-176 AFI validation surface anchor drift")
text = text.replace(old, new, 1)

old_return = """      title, required_terms: source.required_terms, required_terms_missing: missing,
"""
new_return = """      title,
      validation_surfaces: source.key === 'afi-catalog' ? ['visible-innerText', 'same-page-body-textContent'] : ['visible-innerText'],
      required_terms: source.required_terms, required_terms_missing: missing,
"""
if text.count(old_return) != 1:
    raise SystemExit("UC-176 AFI receipt surface anchor drift")
text = text.replace(old_return, new_return, 1)

OUTPUT.write_text(text, encoding="utf-8")
print(f"PASS — materialized exact-blob UC-176 AFI same-page DOM repair at {OUTPUT}")
