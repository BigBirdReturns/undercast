...
const siteAssets=await Promise.all(["index.html","recognition.html","coverage.html","constellation.html","404.html","release-1.0.json","assets/site-shell.css","assets/site-navigation.js","assets/record-page.css","assets/coverage.css","assets/constellation.css"].map(fileMeta));
...
const archive={
  ...
  projections:{lean_index:leanIndex,shard_manifest:{...(await fileMeta("data/shard-manifest.json")),count:shardManifest.count,shards:shardManifest.shards},entities:entityMeta,species:speciesMeta,search:searchMeta,media_live:mediaMeta,quality:qualityMeta,census:censusMeta},
  vocabularies:{conditions:conditionVocabMeta,species:speciesVocabMeta},
  routes:{record:"records/{id}/",interactive_record:"recognition.html#{id}",wall_record:"index.html#{id}",filtered_wall:"index.html?{query}",species_filtered_wall:"index.html?species={singular_label}",constellation:"constellation.html?id={constellation_id}&node={node_id}",recovery:"404.html",retired_ids:"canonical.tombstones"},
  discovery:{robots:"robots.txt",sitemap:"sitemap.xml",dataset:"data/dataset.jsonld",crawler_guide:"CRAWLERS.md"},
  policies:{truth:"Canonical records, image-source ledger and tombstones are maintained evidence. Every projection is disposable.",evidence:"Do not promote inferred conditions or identities to fact. Evidence-scoped claims require a source URL.",species:"Species navigation is generated only from maintained exact census categories and exact performer-role joins. Unknown performers and unresolved performance modes remain explicit.",image_crop:"Fixed frames default to upper-center. Optional still.focus and portrait.focus are shared semantic overrides.",cache:"Every cached artifact is disposable. sha256 and bytes describe the exact published UTF-8/LF payload.",privacy:"No visitor profile, search history, or server-side session is collected."},
...
const urls=[`${ORIGIN}/`,`${ORIGIN}/recognition.html`,`${ORIGIN}/coverage.html`,`${ORIGIN}/constellation.html`,...specimens.map(record=>`${ORIGIN}/records/${record.id}/`),...(tombstones.records||[]).filter(record=>record.status!=="merged").map(record=>`${ORIGIN}/records/${record.id}/`)];
