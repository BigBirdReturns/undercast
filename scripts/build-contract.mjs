...
const comparisonReviews=JSON.parse(await readFile("data/comparison-reviews.json","utf8"));
const comparisonQueue=JSON.parse(await readFile("data/comparison-queue.json","utf8"));
...
const canonicalComparisonReviews=await fileMeta("data/comparison-reviews.json");
...
const speciesVocabMeta=await fileMeta("data/vocabularies/species.json");
const speciesMeta=await fileMeta("data/species.json");
const qualityMeta=await fileMeta("data/quality.json");
const comparisonQueueMeta=await fileMeta("data/comparison-queue.json");
...
const siteAssets=await Promise.all(["index.html","recognition.html","comparison-review.html","coverage.html","constellation.html","404.html","assets/site-shell.css","assets/record-page.css","assets/coverage.css","assets/constellation.css"].map(fileMeta));
const schemas=Object.fromEntries(await Promise.all([
  ["archive","schema/archive.schema.json"],["specimen","schema/specimen.schema.json"],["source","schema/source.schema.json"],["comparison_reviews","schema/comparison-reviews.schema.json"],["comparison_queue","schema/comparison-queue.schema.json"],["entities","schema/entities.schema.json"],["species","schema/species.schema.json"],["species_vocabulary","schema/species-vocabulary.schema.json"],["constellations","schema/constellations.schema.json"],["census_manifest","schema/census-manifest.schema.json"],["census_test","schema/census-test.schema.json"],["tombstones","schema/tombstones.schema.json"]
...
const archive={
  ...
  canonical:{records:{...canonicalRecords,schema:"schema/specimen.schema.json",count:specimens.length,content_sha256:shardManifest.source_sha256},sources:{...canonicalSources,schema:"schema/source.schema.json",count:sources.length},comparison_reviews:{...canonicalComparisonReviews,schema:"schema/comparison-reviews.schema.json",count:comparisonReviews.reviews.length},constellations:{...canonicalConstellations,schema:"schema/constellations.schema.json",count:constellations.constellations.length,nodes:constellations.nodes.length,edges:constellations.edges.length},ds9_changeling_census:{...ds9ChangelingCensus,constellation_id:"constellation:ds9-changeling-performers"},census_manifest:{...canonicalCensusManifest,schema:"schema/census-manifest.schema.json"},tombstones:{...tombstoneMeta,count:(tombstones.records||[]).length}},
  schemas,
  path_bases:{contract_paths:"repository root",shard_manifest_children:"data/"},
  projections:{lean_index:leanIndex,shard_manifest:{...(await fileMeta("data/shard-manifest.json")),count:shardManifest.count,shards:shardManifest.shards},entities:entityMeta,species:speciesMeta,search:searchMeta,media_live:mediaMeta,quality:qualityMeta,comparison_queue:{...comparisonQueueMeta,summary:comparisonQueue.summary},census:censusMeta},
  vocabularies:{conditions:conditionVocabMeta,species:speciesVocabMeta},
  routes:{record:"records/{id}/",interactive_record:"recognition.html#{id}",comparison_review:"comparison-review.html#{id}",wall_record:"index.html#{id}",filtered_wall:"index.html?{query}",species_filtered_wall:"index.html?species={singular_label}",constellation:"constellation.html?id={constellation_id}&node={node_id}",recovery:"404.html",retired_ids:"canonical.tombstones"},
  discovery:{robots:"robots.txt",sitemap:"sitemap.xml",dataset:"data/dataset.jsonld",crawler_guide:"CRAWLERS.md"},
  policies:{truth:"Canonical records, image-source ledger and tombstones are maintained evidence. Every projection is disposable.",evidence:"Do not promote inferred conditions or identities to fact. Evidence-scoped claims require a source URL.",species:"Species navigation is generated only from maintained exact census categories and exact performer-role joins. Unknown performers and unresolved performance modes remain explicit.",image_crop:"Fixed frames default to upper-center. Optional still.focus and portrait.focus are shared semantic overrides. Optional image.comparison is a human-reviewed Recognition Loop-only alignment profile; it may zoom in, never out, and must not rescue an incompatible source pair.",comparison:"A visual slider is available only when its exact still and portrait bytes have an approved human review. Changed bytes become stale; new pairs remain pending; all other states are withheld and publicly discoverable.",cache:"Every cached artifact is disposable. sha256 and bytes describe the exact published UTF-8/LF payload.",privacy:"No visitor profile, search history, or server-side session is collected."},
  web_assets:siteAssets,
};
await writeFile("data/archive.json",JSON.stringify(archive,null,1)+"\n");

const dataset={"@context":"https://schema.org","@type":"Dataset","@id":`${ORIGIN}/#dataset`,identifier:"undercast",name:"UNDERCAST",description:archive.description,url:`${ORIGIN}/`,version:String(archive.version),creator:{"@type":"Organization",name:"UNDERCAST contributors"},isAccessibleForFree:true,keywords:["performers","prosthetics","creature suits","masks","performance capture","voice performance","evidence graph"],distribution:[{"@type":"DataDownload",name:"Canonical specimen catalog",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/specimens.json`},{"@type":"DataDownload",name:"Canonical constellation evidence graph",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/constellations.json`},{"@type":"DataDownload",name:"Archive contract",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/archive.json`},{"@type":"DataDownload",name:"Derived entity index",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/entities.json`},{"@type":"DataDownload",name:"Sourced species navigation",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/species.json`},{"@type":"DataDownload",name:"Franchise census coverage",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/CENSUS-COVERAGE.json`}]};
dataset.distribution.splice(1,0,{"@type":"DataDownload",name:"Canonical comparison review ledger",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/comparison-reviews.json`},{"@type":"DataDownload",name:"Current comparison review queue",encodingFormat:"application/json",contentUrl:`${ORIGIN}/data/comparison-queue.json`});
await writeFile("data/dataset.jsonld",JSON.stringify(dataset,null,1)+"\n");

const urls=[`${ORIGIN}/`,`${ORIGIN}/recognition.html`,`${ORIGIN}/comparison-review.html`,`${ORIGIN}/coverage.html`,`${ORIGIN}/constellation.html`,...specimens.map(record=>`${ORIGIN}/records/${record.id}/`),...(tombstones.records||[]).filter(record=>record.status!=="merged").map(record=>`${ORIGIN}/records/${record.id}/`)];
