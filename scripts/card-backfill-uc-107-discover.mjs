...
const CONTROL = '.github/CARD-BACKFILL-UC-107-DISCOVER.json';
const OUT = process.env.OUT || '/tmp/card-backfill-uc-107-discover';
...
async function inspectPage(context, pageSpec) {
  ...
  try {
    ...
    const caption_receipts = (pageSpec.final_assets || []).map(asset => ({
      asset_key: asset.key,
    ...
    return {
      ...
      final_asset_caption_receipts: caption_receipts
    };
  } catch (error) {
    return {
      status: 'error',
...
assert(control.version === 1 && control.lane === 'card-backfill' && control.record_id === 'UC-107', 'UC-107 discovery scope drift');
assert(control.actor === 'Pedro Pascal' && control.character === 'The Mandalorian' && control.production === 'The Mandalorian' && control.year === 2019 && control.side === 'still', 'UC-107 discovery identity drift');
assert(control.selector_artifact?.artifact_id === 8672920095 && control.scope_artifact?.artifact_id === 8672971563, 'UC-107 discovery custody drift');
assert(control.pages?.length === 3 && control.pages.filter(page => page.strict).length === 3, 'UC-107 page denominator drift');
assert(control.pages[0]?.final_assets?.length === 2 && control.minimum_final_candidates === 2, 'UC-107 final candidate denominator drift');
assert(control.selection_contract?.official_lucasfilm_or_disney_final_frame_required === true && control.selection_contract?.exact_2019_titular_mandalorian_required === true && control.selection_contract?.frame_specific_body_occupant_claim_forbidden === true && control.selection_contract?.canonical_mutation === false, 'UC-107 selection contract drift');
...
try {
  ...
  assert(represented.size === 2, `UC-107 discovery represented only ${represented.size} official final assets`);
  assert(candidates.length >= control.minimum_final_candidates, `UC-107 discovery produced only ${candidates.length} usable candidates`);
  ...
  const manifest = {
    ...
    identity_boundary: {
      character_frame_custody: 'The official press page identifies each retained image as The Mandalorian in the 2019 series.',
      actor_role_custody: 'Lucasfilm separately identifies Pedro Pascal as cast in the title role.',
      prohibited_inference: 'The evidence does not claim that Pascal was the physical occupant of the armor in any selected frame.'
    },
  ...
  await writeJson(join(OUT, 'summary.json'), {
    ...
    candidate_count: candidates.length,
    represented_asset_keys: [...represented].sort(),
    prohibited_inference: manifest.identity_boundary.prohibited_inference,
    candidates: candidates.map(({ page_key, provider, source_page, asset_key, caption, admission_class, expected_subject, declared_url, probe_url, probe_kind, resolved_url, local, mime, bytes, sha256, width, height, repository_matches }) => ({ page_key, provider, source_page, asset_key, caption, admission_class, expected_subject, declared_url, probe_url, probe_kind, resolved_url, local, mime, bytes, sha256, width, height, repository_matches }))
  });
  const cards = candidates.map((row, position) => `<article><img src="${row.local}" alt=""><h2>${position + 1} · ${row.asset_key}</h2><p>${row.provider} · ${row.width}×${row.height} · ${row.bytes} bytes</p><p>${row.caption}</p><p>${row.repository_matches.length ? `duplicate: ${row.repository_matches.join(', ')}` : 'no exact canonical duplicate'}</p><code>${row.sha256}</code></article>`).join('');
  await writeFile(join(OUT, 'review.html'), `<!doctype html><meta charset="utf-8"><style>body{font:14px system-ui;background:#e8e3d9;margin:24px}.sheet{max-width:100%}.grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(360px,1fr));gap:12px}article{background:white;padding:10px}article img{width:100%;height:360px;object-fit:contain;background:#171512}code{display:block;font-size:9px;word-break:break-all}</style><h1>UC-107 · Pedro Pascal / The Mandalorian</h1><p>Candidate-only. Approve only the 2019 titular Mandalorian. Actor-role custody and character-frame custody are separate. Do not infer the frame-specific armor occupant.</p><img class="sheet" src="contact-sheet-final.jpg" alt=""><div class="grid">${cards}</div>`);
  console.log(`PASS — UC-107 official discovery retained ${candidates.length} candidate delivery or original probe(s) across ${represented.size} final frames`);
