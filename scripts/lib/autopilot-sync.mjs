...
function applyDraftFeedback(job, { pendingDrafts, growthRejections, now }, events) {
  if (job.role_on_wall) return;
  ...
  if (pending && job.status !== "resolved") {
    ...
    job.outcome = {
      kind: "draft",
      submitted_at: job.outcome?.submitted_at || pendingDraft?._autopilot?.submitted_at || now,
      submitted_by: job.outcome?.submitted_by || pendingDraft?._autopilot?.submitted_by || "reconciled",
      draft_identity: job.outcome?.draft_identity || draftIdentity(pendingDraft),
      lease_id: job.outcome?.lease_id || pendingDraft?._autopilot?.lease_id || "",
    };
    delete job.lease;
    delete job.next_retry_at;
    return;
...
export function syncState({ coverage, scopes, manifest = { observations: [] }, state = emptyState(), coverageSha256, sourcePaths = {}, drafts = [], specimens = [], growthRejections = [], now = new Date().toISOString() }) {
  ...
  for (const fresh of incoming) {
    ...
    if (!old) {
      const status = fresh.role_on_wall ? "resolved" : fresh.queueable ? "queued" : "attention";
      const job = {
        ...fresh,
        status,
        attempts: 0,
      ...
      applyDraftFeedback(job, { pendingDrafts, growthRejections, now }, events);
      merged.push(job);
      continue;
    }
    ...
    if (fresh.role_on_wall) {
      ...
      if (needsMediaReview) {
        if (job.status !== "merged" || JSON.stringify(job.wall_ids) !== JSON.stringify(fresh.wall_ids)) {
          events.push(event("task.merged", job, now, { wall_ids: fresh.wall_ids }));
        }
        job.status = "merged";
        job.outcome = {
          kind: "merged",
