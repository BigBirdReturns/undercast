...
const CONTROL='.github/CARD-BACKFILL-UC-066-APPLY.json';
const SOURCE_ROOT=process.env.SOURCE_ROOT||'';
const DEST='data/review/card-backfill/UC-066';
...
const expectedFiles=['SHA256SUMS','card-crop-preview.jpg','duplicate-scan.json','manifest.json','review.json','review.md','source-page.png','uc-066-still-candidate.jpg'];

async function loadControl(){
  const control=await readJson(CONTROL);
  assert(control.version===1&&control.lane==='card-backfill'&&control.record_id==='UC-066','UC-066 apply scope drift');
  assert(control.actor==='John Hurt'&&control.character==='John Merrick'&&control.side==='still'&&control.reviewed_role==='second-desk','UC-066 apply authority drift');
  assert(control.render_artifact?.artifact_id===8636124682&&control.render_artifact?.head_sha==='38188fa5a87723095dba1d52b416157f980a2f5c','UC-066 render artifact custody drift');
  ...
  assert(control.ruling?.identity==='expected-subject'&&control.ruling?.presentation==='character-depiction'&&control.ruling?.crop_ruling==='pass-center-focus'&&control.ruling?.canonical_mutation===false,'UC-066 apply ruling drift');
  return control;
}
...
async function materialize(){
  ...
  const files={manifest:'manifest.json',duplicates:'duplicate-scan.json',review:'review.json',reviewMd:'review.md',candidate:'uc-066-still-candidate.jpg',crop:'card-crop-preview.jpg',page:'source-page.png'};
  ...
  assert(sourceManifest.record_id==='UC-066'&&sourceManifest.actor==='John Hurt'&&sourceManifest.character==='John Merrick'&&sourceManifest.side==='still','source manifest identity drift');
  assert(sourceManifest.source?.provider==='Spooky Isles'&&sourceManifest.source?.source_page==='https://www.spookyisles.com/joseph-merrick-elephant-man/','source provenance drift');
  assert(String(sourceManifest.source?.caption||'').includes('John Hurt as Joseph Merrick')&&String(sourceManifest.source?.caption||'').includes('The Elephant Man'),'source caption drift');
  ...
  for(const file of['uc-066-still-candidate.jpg','card-crop-preview.jpg','source-page.png'])await copyFile(join(SOURCE_ROOT,file),join(DEST,file));
  ...
  await writeFile(join(DEST,'review.md'),`# UC-066 reviewed John Merrick still candidate\n\n- **Record:** UC-066\n- **Performer:** John Hurt\n- **Displayed role:** John Merrick\n- **Production:** The Elephant Man (1980)\n- **Source:** [Spooky Isles](${manifest.source.source_page})\n- **Candidate:** \`${control.expected.candidate_sha256}\`\n- **Wall-crop preview:** \`${control.expected.crop_preview_sha256}\`\n- **Identity ruling:** expected subject\n- **Presentation ruling:** character depiction\n- **Crop ruling:** pass — center focus\n- **Canonical mutation:** none\n\n## Visual ruling\n\n${control.ruling.notes.map(note=>`- ${note}`).join('\n')}\n\nThe exact source-pinned 800×450 JPEG is retained unchanged. Four smaller same-composition deliveries remain recorded as rejected visual duplicates. The centered crop preserves the complete Christopher Tucker-designed face and shoulder context. This packet remains evidence-only pending independent canonical acceptance.\n`);
...
async function validate(){
  const control=await loadControl();
  const names=(await readdir(DEST)).sort();assert(JSON.stringify(names)===JSON.stringify([...expectedFiles].sort()),`UC-066 evidence file set drift: ${names.join(', ')}`);
  ...
  assert(manifest.source?.provider==='Spooky Isles'&&manifest.crop_preview?.gravity==='center'&&manifest.rejected_visual_duplicates?.length===4,'source/crop/visual duplicate receipt drift');
  ...
  assert((await fileMeta(join(DEST,'uc-066-still-candidate.jpg'))).sha256===control.expected.candidate_sha256,'candidate bytes drift');
